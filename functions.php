...
if ( ! function_exists( 'geoffreycrofte_setup' ) ) :
	...
	function geoffreycrofte_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'geoffreycrofte_custom_background_args',
				array(
...
function geoffreycrofte_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'geoffreycrofte_content_width', 640 );
}
...
function geoffreycrofte_scripts() {
	wp_enqueue_style( 'geoffreycrofte-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'geoffreycrofte-style', 'rtl', 'replace' );

	wp_enqueue_script( 'geoffreycrofte-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
